...
/**
 * This class is executed periodically and it checks whether there are stalled probes
 */
public class ProbeCheck extends TimerTask {
	...
	String query="select * from Services where ProbeRunning='Y' and NextCheckTime<DATE_SUB(NOW(), INTERVAL 1 HOUR)";
	//String query="select dbid from Services ";

	String messageText="";
	int numberOfStalledProbes = 0;

	PreparedStatement queryPS=null;
	try{
	    queryPS=ParameterStore.getParameterStore().getDb().getConn().prepareStatement(query);
	    ResultSet rs=queryPS.executeQuery();
	    while(rs.next()){
		numberOfStalledProbes=numberOfStalledProbes+1;
		int dbid=rs.getInt("dbid");
		Timestamp nextCheckTime = rs.getTimestamp("NextCheckTime");
		messageText=messageText+dbid+" "+nextCheckTime+"\n";
		System.out.println(new Date()+" "+getClass().getName()+" dbid="+dbid);		
	    }
	    rs.close();	    
	}catch (Exception e){
	    System.out.println(new Date()+" "+getClass().getName()+" error occured when obtaining list of stalled probes");
	...
	if(numberOfStalledProbes>0){
	    // reset the stalled probes
	    int numberOfProbesReset = 0;
	    String resetCommand="UPDATE Services SET ProbeRunning='N' WHERE NextCheckTime<DATE_SUB(NOW(), INTERVAL 1 HOUR)";
	    PreparedStatement resetCommandPS=null;
	    try{
		resetCommandPS=ParameterStore.getParameterStore().getDb().getConn().prepareStatement(resetCommand);
		numberOfProbesReset=resetCommandPS.executeUpdate();
	    }catch (Exception e){
		System.out.println(new Date()+" "+getClass().getName()+" error occured when reseting stalled probes");
		System.out.println(new Date()+" "+getClass().getName()+" resetCommand"+resetCommand);
		System.out.println(new Date()+" "+getClass().getName()+" "+e);
	    }
	    try{
		resetCommandPS.close();
	    }catch (Exception e){
		System.out.println(new Date()+" "+getClass().getName()+"failed to close prepared statement");
		System.out.println(new Date()+" "+getClass().getName()+" resetCommand="+resetCommand);
		System.out.println(new Date()+" "+getClass().getName()+" "+e);
	    ...
	    messageText="The time is: "+new Date()+"\n"+"Found the following stalled probes:\n"+messageText;
	    messageText=messageText+"\n\n";
	    messageText=messageText+"Reset "+numberOfProbesReset+" probes\n";
	    sendAlertToUser("tomw@bnl.gov",numberOfStalledProbes+" stalled probes found",messageText);
	}
    ...
    public String sendAlertToUser(String userEmail, String messageSubject, String messageText){
	String result="";
	Date now = new Date();
